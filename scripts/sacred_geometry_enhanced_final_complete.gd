...
var transition_from_pattern: Node3D
var transition_to_pattern: Node3D

# Signal for pattern transitions
signal pattern_transition_completed(pattern_type)
...
func _update_transition(delta: float):
    transition_progress += delta / PATTERN_TRANSITION_TIME
    ...
    if transition_progress >= 1.0:
        ...
        # Hide old pattern
        if transition_from_pattern:
            transition_from_pattern.visible = false
            
            # Stop particle emission
            if transition_from_pattern in particle_systems:
                var particles = particle_systems[transition_from_pattern]
                if particles and particles.has("main"):
                    particles["main"].emitting = false
        
        # Set new pattern as active
        active_pattern = transition_to_pattern
        ...
        # Emit signal
        for pattern_type in patterns:
            if patterns[pattern_type] == active_pattern:
                emit_signal("pattern_transition_completed", pattern_type)
                break
    else:
        # Update transition
        var t = transition_progress
        
        # Fade out old pattern
        if transition_from_pattern:
            var scale_factor = 1.0 - t * 0.5
            transition_from_pattern.scale = Vector3.ONE * scale_factor
            
            # Update material emission for all child meshes
            for child in transition_from_pattern.get_children():
                if child is MeshInstance3D:
                    var material = child.get_surface_override_material(0)
                    if material and material is StandardMaterial3D:
                        material.emission_energy = PATTERN_EMISSION_MAX * (1.0 - t)
                        material.albedo_color.a = 1.0 - t
        
        # Fade in new pattern
        if transition_to_pattern:
            var scale_factor = 0.5 + t * 0.5
            transition_to_pattern.scale = Vector3.ONE * scale_factor
            
            # Update material emission for all child meshes
            for child in transition_to_pattern.get_children():
                if child is MeshInstance3D:
                    var material = child.get_surface_override_material(0)
                    if material and material is StandardMaterial3D:
                        material.emission_energy = PATTERN_EMISSION_MAX * t
                        material.albedo_color.a = t
...
func transition_to_pattern(pattern_type: int) -> void:
    if not (pattern_type in patterns):
        # Invalid pattern type
        return
    
    if not active_pattern:
        # No active pattern, just activate the new one
    ...
    # Set up transition
    transition_from_pattern = active_pattern
    transition_to_pattern = patterns[pattern_type]
    transition_active = true
    transition_progress = 0.0
    
    # Make new pattern visible
    transition_to_pattern.visible = true
    transition_to_pattern.scale = Vector3.ONE * 0.5
    ...
    # Start particle emission for new pattern
    if transition_to_pattern in particle_systems:
        var particles = particle_systems[transition_to_pattern]
        if particles and particles.has("main"):
            particles["main"].emitting = true

# Public method to set time of day (changes color palette)
func set_time_of_day(time: String) -> void:
    if time in COLOR_PALETTES:
        current_time_of_day = time
        
        # Update all patterns with new colors
        _update_pattern_colors()

# Update pattern colors based on time of day
func _update_pattern_colors() -> void:
    # Recreate all patterns with new colors
    var old_active_pattern = active_pattern
    
    # Store pattern type of active pattern
    var active_pattern_type = -1
    for pattern_type in patterns:
        if patterns[pattern_type] == active_pattern:
            active_pattern_type = pattern_type
            break
    ...
    # Create new patterns with updated colors
    _create_all_patterns()
    
    # Restore active pattern if there was one
    if active_pattern_type >= 0:
        activate_pattern(active_pattern_type)
