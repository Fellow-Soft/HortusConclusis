...
# Enhanced systems
var CinematicEffectsEnhanced = load("res://scripts/cinematic_effects_enhanced.gd")
var AtmosphereController = load("res://scripts/atmosphere_controller_enhanced.gd")
var MeditationDisplay = load("res://scripts/meditation_display_enhanced.gd")
var SacredGeometry = load("res://scripts/sacred_geometry_enhanced.gd")
...
func _initialize_cinematic():
    ...
    atmosphere_controller = AtmosphereController.instantiate()
    add_child(atmosphere_controller)
    meditation_display = MeditationDisplay.instantiate()
    add_child(meditation_display)
    sacred_geometry = SacredGeometry.instantiate()
    add_child(sacred_geometry)
    cinematic_effects = CinematicEffectsEnhanced.instantiate()
    add_child(cinematic_effects)
    plant_generator = PlantGenerator.instantiate()
...
func _start_cinematic():
    ...
    if meditation_display:
        var meditation_text = ""
        for i in range(MEDITATIONS[current_meditation].size()):
            meditation_text += MEDITATIONS[current_meditation][i]
            if i < MEDITATIONS[current_meditation].size() - 1:
                meditation_text += "\n"
...
func _update_sacred_patterns(delta: float):
    if sacred_geometry:
        sacred_geometry.update_patterns(delta)
...
func _highlight_garden_types():
    if sacred_geometry:
        sacred_geometry.highlight_garden_types()

# Activate sacred patterns sequence
func _activate_sacred_patterns_sequence():
    if sacred_geometry:
        sacred_geometry.activate_patterns_sequence()
...
func _on_time_changed(time: String):
    if sacred_geometry:
        sacred_geometry.update_time_of_day(time)
